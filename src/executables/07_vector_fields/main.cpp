...
//Uniform_grid_params grid_params(40.0f, 40.0f, 40.0f, 1.0f, 1.0f, 1.0f, 15);

int FLOCK_SIZE = grid_params.total_cell_count;
enum
{
	WORKGROUP_SIZE = 128,
	NUM_WORKGROUPS = 1, //ja wenn das auf 1 ist, dann ist klar, dass nur 128 partikel einsortiert werden, du nullpe
...
private:
	...
	void createComputeDescriptorSet()
	{
		...
		std::vector<VkDescriptorPoolSize> poolSizes =
		{
			...
			descriptorPoolSize(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 24)  //ssbo count * 3
		};
		...
		gridNeighborhoodDataBufferInputInfo.range = neighbor_amount * FLOCK_SIZE * sizeof(int);
		...
		gridNeighborhoodCounterBufferInputInfo.range = FLOCK_SIZE * sizeof(int);
	...
	void createGridResetPipeline()
	{
		...
		auto compShaderCode = readFile("../src/shaders/vector_field/grid_reset.comp.spv");
	...
	void createGridSetupPipeline()
	{
		...
		auto compShaderCode = readFile("../src/shaders/vector_field/grid_setup.comp.spv");
	...
	void createGridFindNeighborPipeline()
	{
		...
		auto compShaderCode = readFile("../src/shaders/vector_field/grid_find_neighbors.comp.spv");
	...
	void createVectorFieldPipeline()
	{
		...
		auto compShaderCode = readFile("../src/shaders/vector_field/vector_field.comp.spv");
	...
	void createComputePipeline()
	{
		...
		auto compShaderCode = readFile("../src/shaders/vector_field/shader.comp.spv");
	...
	{
		auto vertShaderCode = readFile("../src/shaders/vector_field/shader.vert.spv");
		auto fragShaderCode = readFile("../src/shaders/vector_field/shader.frag.spv");
		...
		attributeDescriptions[4].offset = offsetof(Flock, color);
	...
	void createStorageBuffers()
	{
		...
		/*boidBuffer[0].position = glm::vec3(-2.0f, -2.0f, -2.0f);
		boidBuffer[1].position = glm::vec3(-2.0f, -2.0f, -2.0f);
		boidBuffer[2].position = glm::vec3(-2.0f, -2.0f, -2.0f);
		boidBuffer[3].position = glm::vec3(-2.0f, -2.0f, -2.0f);
		boidBuffer[4].position = glm::vec3(-2.0f, -2.0f, -2.0f);

		boidBuffer[5].position = glm::vec3(0.0f, 0.0f, 0.0f);
		boidBuffer[6].position = glm::vec3(0.0f, 0.0f, 0.0f);
		boidBuffer[7].position = glm::vec3(0.0f, 0.0f, 0.0f);
		boidBuffer[8].position = glm::vec3(0.0f, 0.0f, 0.0f);
		boidBuffer[9].position = glm::vec3(0.0f, 0.0f, 0.0f);

		boidBuffer[10].position = glm::vec3(2.0f, 2.0f, 2.0f);
		boidBuffer[11].position = glm::vec3(2.0f, 2.0f, 2.0f);
		boidBuffer[12].position = glm::vec3(2.0f, 2.0f, 2.0f);
		boidBuffer[13].position = glm::vec3(2.0f, 2.0f, 2.0f);
		boidBuffer[14].position = glm::vec3(2.0f, 2.0f, 2.0f);*/
		...
		for (int i = 0; i < grid_params.cell_count.x; i++)
		{
			for (int j = 0; j < grid_params.cell_count.y; j++)
			{
				...
				{
					if (count < grid_params.total_cell_count && count < FLOCK_SIZE)
					{
						boidBuffer[count].position = glm::vec3((grid_params.grid_min.x + offset_x) + (i * grid_params.cell_size.x),
							(grid_params.grid_min.y + offset_y) + (j * grid_params.cell_size.y),
							(grid_params.grid_min.z + offset_z) + (k * grid_params.cell_size.z));
		...
		{
			//boid.position = glm::vec3(2.0f, 2.0f, 2.0f);
			//boid.position = (random_vec3(0.1f, 0.9f)) * glm::vec3(0.5f) * 300.0f;
			//boid.position = glm::vec3(0.0f);
			boid.velocity = (glm::vec3(0.01) - glm::vec3(0.5f));
			boid.color = glm::vec4(1.0f, 1.0f, 1.0f, 0.0f);
		}
	...
	void createGridBuffer()
	{
		...
		VkDeviceSize neighborhood_data_size = neighbor_amount * FLOCK_SIZE * sizeof(int);
		...
		createBuffer(FLOCK_SIZE * sizeof(int), VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, storageBuffers.grid_neighborhood_counter, gridNeighborhoodCounterBufferMemory);
	...
	void createCommandBuffers()
	{
		...
		for (size_t i = 0; i < commandBuffers.size(); i++)
		{
			...
			vkCmdBindVertexBuffers(commandBuffers[i], 1, 1, ssbos, offsets);
